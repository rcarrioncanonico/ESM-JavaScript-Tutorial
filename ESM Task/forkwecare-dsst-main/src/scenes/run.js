...
import sendData from "../gameobjects/sendData.js";
...
export default class Run extends Phaser.Scene {
    ...
    create () {
        ...
        //==== OBJECTS TO INCLUDE ====//
        this.gameUI = new GameUI({ scene: this , symbolSet: this.symbolSet, symbolOrder: this.symbolOrder, debug: false });
        ...
        this.trial.event.once("complete", () => {
            ...
            this.message.show({ message: true, button: false });

            // Send the data after the final screen is shown
            this.time.delayedCall(5000, () => sendData());
        });
    }
    ...
    setStimuli() {
        ...
        const setPerHour = [
            [ hour >= 8 && hour <= 11, symbolSets[0] ],
            [ hour > 11 && hour <= 13, symbolSets[1] ],
            [ hour > 13 && hour <= 15, symbolSets[2] ],
            [ hour > 15 && hour <= 17, symbolSets[3] ],
            [ hour > 17 && hour <= 20, symbolSets[4] ],
        ]
