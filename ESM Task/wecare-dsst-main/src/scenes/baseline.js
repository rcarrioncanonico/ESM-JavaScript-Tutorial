...
import FeedbackHandler from "../gameobjects/feedbackHandler.js";
...
export default class Baseline extends Phaser.Scene {
    ...
    create () {
        ...
        this.stimulusInterval = 500; // Wait 500 ms between stimuli
        //==============================//
        //==============================//
        ...
        this.trial = new TrialHandler({ 
            ...
            stimulusType: "text",
            interStimulusInterval: this.stimulusInterval,
        ...
        //==== OBJECTS TO INCLUDE ====//
        this.gameUI = new GameUI({ scene: this, debug: false });
        ...
        //this.feedback = new FeedbackHandler({ scene: this, x: Boot.canvasCenterX, y: 0.15 * Boot.canvasHeight, animate: true});
        ...
        this.countdown.event.once("complete", () => {
            this.gameUI.show({ trial: this.trialType });
            this.gameUI.recordResponse(true); // Start recording responses

            // ==== Check for response and show feedback ====
            this.events.on("playerResponse", () => {
                if (this.data.get("response.correct") === 1) {
                    this.stimulusBox.setTint(0x00ff12);
                    //this.feedback.show({ response: "correct", message: "goed!"});
                } else {
                    this.stimulusBox.setTint(0xff2c2c);
                    //this.feedback.show({ response: "wrong", message: "fout"});
                }
                // Reset the tint after interval
                this.time.delayedCall(this.stimulusInterval, () => {this.stimulusBox.clearTint()});
            });
