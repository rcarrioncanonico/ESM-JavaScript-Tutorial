...
export default class GameUI {
    constructor({ scene, symbolSet = "p", symbolOrder = [1, 2, 3, 4, 5, 6, 7, 8, 9], debug = false }) {
        ...
        this.symbols = [];
        this.numbers = [];
        this.answerPairs = {}; // This is important to get the button response mathced to the symbol in the key
        ...
        for (let i = 1; i <= 9; i++) {
            ...
            if (i === 9) {
                ...
                for (let i = 1; i <= 9; i++) {
                    ...
                    // Add answerPair
                    this.answerPairs[this.symbolOrder[i - 1]] = this.symbols[i - 1].frame.name;
    ...
    recordResponse(activate = true) {
        if (activate) {
            ...
            for (let i = 0; i < this.scene.buttons.length; i++) {
                const button = this.scene.buttons.getAt(i); // Get the buttons from the container
                button.setActive(true);
                button.callback = () => {
                    console.log("button pressed:", button.labelText);
                    // ==== Save to temp data for writing to jsPsych data file
                    // Check which answer should be correct for the stimulus shown
                    const stimKey = Object.keys(this.answerPairs).find(key => {
                        return this.answerPairs[key] === this.scene.data.get("stim.type");
                    });

                    this.scene.data.set("response.type", button.labelText);
                    this.scene.data.set("response.onset", Math.round(this.scene.time.now)); // Rounded to the nearest integer
                    this.scene.data.set("response.RT", Math.round(this.scene.time.now) - this.scene.data.get("stim.onset"));
                    
                    // Check if the responses are correct or incorrect
                    // For baseline, there is no response key. Only for run
                    if (this.scene.scene.key == "baseline") {
                        this.scene.data.set("stim.key", null);
                        this.scene.data.set("response.key", null);
                        this.scene.data.get("stim.type") == this.scene.data.get("response.type") ? this.scene.data.set("response.correct", 1) : this.scene.data.set("response.correct", 0);
                    } else {
                        this.scene.data.set("stim.key", stimKey);
                        this.scene.data.set("response.key", this.answerPairs[button.labelText]); // Get the name of the correct symbol from order of symbol key
                        this.scene.data.get("stim.type") == this.scene.data.get("response.key") ? this.scene.data.set("response.correct", 1) : this.scene.data.set("response.correct", 0);
                    }
